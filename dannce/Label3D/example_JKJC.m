...
parfor nVid = 1:numel(vidPaths)
    ...
    for frame = 1:max(framesToLabel)
        ...
        if mod(frame,10)==0
            sprintf('Processing frame %d / %d', frame, video_temp.NumFrames)
        end
...
%% Show COM prediction
com = load([projectFolder 'dannce/com3d0.mat']);
com2 = repmat(com.com(framesToLabel,:), 1, 1, 22);
