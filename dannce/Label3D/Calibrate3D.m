classdef Calibrate3D < Animator
    ...
    methods
        function obj = Calibrate3D(varargin)
        ...
        function translateCameraX(obj, px)
            idx = obj.Calibrate;
            obj.origCamParams{idx}.t(1) = obj.origCamParams{idx}.t(1) + px;
            [obj.cameraParams, obj.orientations, obj.locations] = ...
            obj.loadcamParams(obj.origCamParams);
            obj.cameraPoses = obj.getCameraPoses();
        ...
        function translateCameraY(obj, px)
            idx = obj.Calibrate;
            obj.origCamParams{idx}.t(2) = obj.origCamParams{idx}.t(2) + px;
            [obj.cameraParams, obj.orientations, obj.locations] = ...
            obj.loadcamParams(obj.origCamParams);
            obj.cameraPoses = obj.getCameraPoses();
            obj.reprojectPoints(obj.frameInds(obj.frame));
        ...
        function rotateCameraX(obj, ang)
            idx = obj.Calibrate;
            R = [cosd(ang) -sind(ang) 0; sind(ang) cosd(ang) 0; 0 0 1];
            obj.origCamParams{idx}.r = obj.origCamParams{idx}.r * R;
            [obj.cameraParams, obj.orientations, obj.locations] = ...
            obj.loadcamParams(obj.origCamParams);
            obj.cameraPoses = obj.getCameraPoses();
            obj.reprojectPoints(obj.frameInds(obj.frame));
        ...
        function rotateCameraZ(obj, ang)
            idx = obj.Calibrate;
        ...
        function rotateCameraY(obj, ang)
            idx = obj.Calibrate;
...
%             end
        ...
        function keyPressCallback(obj,source,eventdata)
            ...
            switch keyPressed
                ...
                case 'numpad3'
                    obj.rotateCameraZ(-1);
                case 'y'
                    obj.translateCameraY(-1);
                case 'n'
                    obj.translateCameraY(1);
                case 'g'
                    obj.translateCameraX(-1);
                case 'j'
                    obj.translateCameraX(1);
                case 'u'
                    obj.resetFrame();
        ...
        function returnCalibrate(obj)
            if isempty(obj.reference_pts)
                disp('saving reference pts')
                obj.reference_pts = reshape(obj.camPoints(1:5,obj.Calibrate,:,1),5,2);
            else
                disp('saving calibrate pts')
                obj.calibrate_pts = reshape(obj.camPoints(1:5,obj.Calibrate,:,1),5,2);
            end
        end
        ...
        function exportDannce(obj, varargin)
            ...
            for nCam = 1:nCameras
                ...
                if ~obj.undistortedImages
                    ...
                    data_2D = worldToImage(cp, cp.trices,...
                        cp.TranslationVectors, allpts);RotationMa
                end
