...
% Input Parameters
clear
basedir = '/Users/hyungju/Desktop/hyungju/Result/functional-stn/210713/validation_move/'
numcams = 4;
squareSize = 22.0; % Size of Checkerboard squares in mm
ext = '.mp4';
maxNumImages = 100;
videoName = '0';
...
for kk = 1:numcams
    ...
    tic
    video_temp = VideoReader([basedir 'Camera' num2str(kk) filesep videoName '.mp4']);
    ...
    [params_individual{kk},pairsUsed,estimationErrors{kk}] = estimateCameraParametersPar(imagePoints{kk}(:,:,imageNums{kk}),worldPoints, ...
        'ImageSize',imageSize,'EstimateSkew',false,'EstimateTangentialDistortion',false,...
...
for kk=1:numcams
    imFiles1 = VideoReader([basedir 'Camera' num2str(kk) filesep videoName '.mp4'],'CurrentTime', 30); 
    figure(kk);
